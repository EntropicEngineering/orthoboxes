...
#define _SIMPLE_HID_DEVICE_H
	...
	/* Macros */
		...
		#define SIMPLE_HID_APPLICATION_COLLECTION 0x0001
		#endif
		#ifndef SIMPLE_HID_ARRAY
		#define SIMPLE_HID_ARRAY 0x0000
		#endif
		 ...
		 *
		 */
		#define SIMPLE_HID_DESCRIPTOR_CUSTOM(PAGE, APPLICATION, ARRAY, OBJECT, BITS, UINT, INT, FLOAT, UTF8) \
			SIMPLE_HID_PLATFORM_DESCRIPTOR_SIZE, DTYPE_DeviceCapability,\
			DCTYPE_Platform, \
			/* Reserved */ 0, \
			SIMPLE_HID_PLATFORM_UUID, \
			BYTES_LE(SIMPLE_HID_VERSION), \
			BYTES_LE(PAGE), BYTES_LE(APPLICATION), BYTES_LE(ARRAY), BYTES_LE(OBJECT), BYTES_LE(BITS), BYTES_LE(UINT), BYTES_LE(INT), BYTES_LE(FLOAT), BYTES_LE(UTF8)
